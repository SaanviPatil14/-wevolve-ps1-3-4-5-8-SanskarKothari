...
export function calculateMatch(candidate: Candidate, job: Job): MatchResult {
  // Normalize candidate fields to safe defaults when data is missing from the DB
  const cand = {
    skills: candidate?.skills || [],
    preferred_locations: candidate?.preferred_locations || [],
    expected_salary: (candidate && typeof candidate.expected_salary === 'number') ? candidate.expected_salary : 0,
    experience_years: (candidate && typeof candidate.experience_years === 'number') ? candidate.experience_years : 0,
    preferred_roles: candidate?.preferred_roles || [],
    // keep reference to candidate details for UI
    ...candidate
  } as Candidate;

  const skills = calculateSkillScore(cand.skills, job.required_skills);
  const loc = calculateLocationScore(
    cand.preferred_locations,
    job.location
  );
  const sal = calculateSalaryScore(cand.expected_salary, job.salary_range);
  const exp = calculateExperienceScore(
    cand.experience_years,
    job.experience_required
  );
  const role = calculateRoleScore(cand.preferred_roles, job.title);

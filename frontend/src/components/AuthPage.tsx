...
import { auth } from "../firebase"; // <--- IMPORT AUTH FOR GOOGLE CHECK
...
const AuthPage = () => {
  ...
  // --- ALLOWED EMPLOYER EMAILS ---
  const ALLOWED_EMPLOYER_EMAILS = [
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // --- RESTRICTION CHECK (MANUAL LOGIN/SIGNUP) ---
      // If they are trying to be an Employer, check if email is allowed
      if (userType === "employer") {
        if (!ALLOWED_EMPLOYER_EMAILS.includes(email)) {
          alert(
            "Access Denied: Employer access is restricted to authorized administrators only."
          );
          setLoading(false);
      ...
      if (error.message.includes("Email not verified")) {
        const wantResend = window.confirm(
          "Your email is not verified yet. Check your spam folder. Would you like to resend the verification link?"
        );
        if (wantResend) {
          try {
  ...
  const handleGoogle = async () => {
    try {
      ...
      // --- RESTRICTION CHECK (GOOGLE LOGIN) ---
      // If the resulting role is Employer, check the actual signed-in email
      if (role === "employer") {
        const currentUserEmail = auth.currentUser?.email;

        if (!ALLOWED_EMPLOYER_EMAILS.includes(currentUserEmail)) {
          alert(
            "Access Denied: Employer access is restricted to authorized administrators only."
          );

...
import { calculateMatch } from "./services/engine"; // Used for Employer side legacy
import { fetchJobMatches } from "./services/matchingService"; // <--- NEW: Python Backend Matching
...
import SkillsGapAnalysis from "./components/pages/SkillsGapAnalysis"; // <--- NEW: Gap Analysis Page
...
const App: React.FC = () => {
  ...
  const [activeCandidate, setActiveCandidate] =
    useState<Candidate>(INITIAL_CANDIDATE);
  ...
  const runEngine = useCallback(async () => {
    // A. Candidate View: Use Python Backend (Multi-Factor Engine)
    if (userRole === "candidate") {
      if (jobs.length === 0) return;

      // Call the new service that talks to your Python backend
      const results = await fetchJobMatches(activeCandidate, jobs);

      setMatches(results);
      if (results.length > 0 && !selectedMatch) setSelectedMatch(results[0]);
    }

    // B. Employer View: Use Legacy Local Engine (or update later)
    else {
      ...
      try {
        ...
        console.error("Error running employer engine:", err);
      }
    }
  }, [userRole, activeCandidate, jobs, selectedJob, selectedMatch]);

  // Run engine when dashboard loads
  useEffect(() => {
  ...
  return (
    <div
      className={`min-h-screen bg-slate-50 pb-20 transition-colors duration-500 ${
        userRole === "candidate" ? "bg-emerald-50/20" : "bg-indigo-50/20"
      }`}
    >
      {showHeader && (
      ...
      <main className={showHeader ? "max-w-7xl mx-auto px-6 py-8" : ""}>
        <Routes>
          <Route
            path="/"
            element={<LandingPage onStart={() => navigate("/auth")} />}
          />
          ...
          <Route path="/analysis" element={<SkillsGapAnalysis />} />
        </Routes>
      </main>
    </div>
  );
